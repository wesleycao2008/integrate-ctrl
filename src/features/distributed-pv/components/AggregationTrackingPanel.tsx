        ...
        if (entry.dataKey === 'upper') {
          return (
            <p key={index} className="text-slate-400">
              可行运行域：
              <span className="text-slate-200">{lower ?? '-'} ~ {upper ?? '-'}</span>
            </p>
          )
        }
        return (
  ...
  return (
    <section className="h-full rounded-2xl border border-slate-800 bg-slate-900/60 p-4 shadow-lg shadow-slate-950/40">
        ...
        <div className="flex items-center gap-3 text-xs">
          ...
          {mode === 'intraDay' && (
            <div className="flex flex-col items-end">
              <p className="text-[11px] text-slate-400">可视范围（小时）</p>
              <div className="mt-1 flex items-center gap-2">
                <input
                  type="range"
                  min={1}
          ...
          {mode === 'dayAhead' && (
            <div className="flex flex-col items-end">
              <p className="text-[11px] text-slate-400">视图</p>
      ...
      <div className="grid gap-3 md:grid-cols-[minmax(0,3fr)_minmax(0,1.1fr)]">
        ...
        <div className="flex flex-col justify-between rounded-xl border border-slate-800 bg-slate-900/80 p-3 text-xs">
          <h3 className="mb-2 font-medium text-slate-100">实时误差监视</h3>
          <dl className="space-y-2">
            <div className="flex items-center justify-between">
              <dt className="text-slate-400">当前误差 ε(t)</dt>
              <dd className={`font-semibold ${isAlarm ? 'text-amber-300' : 'text-emerald-300'}`}>
                {currentError.toFixed(1)} kW
              </dd>
            </div>
            <div className="flex items-center justify-between">
              <dt className="text-slate-400">最大绝对误差 max|ε|</dt>
              <dd className="font-semibold text-slate-100">{maxAbsError.toFixed(1)} kW</dd>
            </div>
            <div className="flex items-center justify-between">
              <dt className="text-slate-400">均方根误差 RMSE</dt>
