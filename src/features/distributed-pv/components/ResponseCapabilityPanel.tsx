...
const ResponseCapabilityPanel: FC = () => {
  ...
  return (
    <section className="space-y-4 rounded-2xl border border-slate-800 bg-slate-900/60 p-4 shadow-lg shadow-slate-950/40">
      <header className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex items-center gap-2 text-[11px]">
          <span className="text-slate-400">视图：</span>
          <button
            type="button"
            onClick={() => setSubTab('dispatcher')}
            className={`rounded-full px-3 py-1 ${
              subTab === 'dispatcher'
                ? 'bg-sky-500/20 text-sky-200 border border-sky-500/70'
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-3 lg:grid-cols-[minmax(0,2.1fr)_minmax(0,3fr)]">
                ...
                >
                  <div className="space-y-0.5">
                    ...
                    <div className="flex items-center gap-3 text-[11px] text-slate-300">
                      ...
                      <span className="text-slate-400">不确定性：±{f.uncertaintyKw} kW</span>
                    </div>
                  </div>
                  <span className="text-[10px] text-slate-500">ID: {f.id}</span>
                </button>
              )
            })}
          </div>
        ...
        {/* 聚合能力时间曲线（未来 4 小时） */}
        <section className="flex flex-col rounded-xl border border-slate-800 bg-slate-950/70 p-3 text-xs">
          <header className="mb-2 flex items-center justify-between">
            <div>
      ...
      <div className="grid gap-3 lg:grid-cols-[minmax(0,2.2fr)_minmax(0,2.5fr)]">
        ...
        <section className="rounded-xl border border-slate-800 bg-slate-950/70 p-3 text-xs">
          ...
          <div className="max-h-44 overflow-auto rounded-lg border border-slate-800 bg-slate-950/80">
            <table className="min-w-full border-separate border-spacing-0 text-[11px]">
              <thead className="sticky top-0 bg-slate-900">
                <tr>
                  {['排名', '台区', '可增能力 (kW)', '裕度 (%)', '状态'].map((col) => (
                    <th key={col} className="border-b border-slate-800 px-2 py-1.5 text-left font-medium text-slate-300">
                      {col}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {ranking.map((row, idx) => {
                  ...
                  return (
                    <tr key={row.name} className="border-b border-slate-800/70 hover:bg-slate-900/60">
                      <td className="px-2 py-1.5 text-slate-200">{idx + 1}</td>
        ...
        {/* 下达指令辅助 */}
        <section className="flex flex-col justify-between rounded-xl border border-slate-800 bg-slate-950/70 p-3 text-xs">
          <div>
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              <label className="space-y-1">
                <span className="text-slate-300">计划上调目标 ΔP_up (kW)</span>
                <input
                  type="number"
                  min={0}
                  value={plannedTarget}
              ...
              <div className="space-y-1">
                <span className="text-slate-300">全网总可增能力</span>
                <div className="rounded-lg border border-slate-800 bg-slate-900 px-2 py-1.5">
                  ...
                  <p className="mt-0.5 text-[11px] text-slate-500">最薄弱台区：{minUpFeeder.name}（{minUpFeeder.upCapacityKw} kW）</p>
                </div>
              </div>
            </div>

            <div className="mt-3 rounded-lg border border-slate-800 bg-slate-950/80 p-2.5 text-[11px]">
              {!isTargetExceeded ? (
    ...
    <div className="space-y-4 h-full">
      ...
      <div className="grid gap-3 xl:grid-cols-[minmax(0,2fr)_minmax(0,3fr)]">
        ...
        <section className="rounded-xl border border-slate-800 bg-slate-950/70 p-3 text-xs">
          ...
          <div className="flex flex-col gap-3 md:flex-row">
            {/* 简化单线拓扑，使用圆点表示节点 */}
            <div className="flex-1 rounded-lg border border-slate-800 bg-slate-950/90 p-3">
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center justify-center rounded-full border text-[10px] font-mono text-slate-900 shadow ${color} ${
                        selectedInverterId === inv.id ? 'ring-2 ring-sky-300' : ''
                      }`}
            ...
            {/* 选中节点详情 */}
            <div className="w-full max-w-xs rounded-lg border border-slate-800 bg-slate-950/90 p-3 text-[11px]">
              <h4 className="mb-1 font-medium text-slate-100">节点详情</h4>
              {selectedInverter ? (
                <dl className="space-y-1.5 text-slate-200">
                  <div className="flex justify-between">
                    <dt className="text-slate-400">逆变器 ID</dt>
                  ...
                  <div className="flex justify-between">
                    <dt className="text-slate-400">P_cur / P_avail</dt>
                    <dd>
                      {selectedInverter.pCur} / {selectedInverter.pAvail} kW
                    </dd>
                  </div>
                  <div className="flex justify-between">
                    <dt className="text-slate-400">ΔP_up / ΔP_down</dt>
                    <dd>
                  ...
                  {showReactive && (
                    <div className="flex justify-between">
                      <dt className="text-slate-400">无功范围 Q_min~Q_max</dt>
                      <dd>
                  ...
                  <div className="flex justify-between">
                    <dt className="text-slate-400">响应评分</dt>
                ...
                <p className="text-slate-400">请在左侧单线图或下方表格中选择一个逆变器查看详细能力信息。</p>
              )}
            </div>
          </div>
        </section>
        ...
        {/* 逆变器能力列表与筛选 */}
        <section className="rounded-xl border border-slate-800 bg-slate-950/70 p-3 text-xs h-full flex flex-col">
          ...
          <div className="h-full min-h-0 overflow-auto rounded-lg border border-slate-800 bg-slate-950/80">
            <table className="min-w-full border-separate border-spacing-0 text-[11px]">
              <thead className="sticky top-0 bg-slate-900">
                <tr>
                  {[
              ...
              <tbody>
                {filteredInverters.map((inv) => {
                  ...
                  return (
                    <tr key={inv.id} className={`border-b border-slate-800/70 hover:bg-slate-900/70 ${selectedInverterId === inv.id ? 'bg-sky-500/10' : ''}`}>
                      ...
                      <td className="px-2 py-1.5"><span className="rounded-full bg-slate-800 px-2 py-0.5 font-mono text-emerald-300">{inv.deltaUp}</span></td>
                      ...
                      <td className="px-2 py-1.5">
                        {isAbnormal ? (
                          ...
                          <button type="button" onClick={() => setSelectedInverterId(inv.id)} className="rounded-full border border-sky-500/60 bg-sky-500/10 px-2 py-0.5 text-[10px] text-sky-200 hover:bg-sky-500/20">查看详情</button>
                        )}
                      </td>
                    </tr>
                  )
      ...
      <div className="grid gap-3 xl:grid-cols-[minmax(0,2.1fr)_minmax(0,3fr)]">
        ...
        <section className="rounded-xl border border-slate-800 bg-slate-950/70 p-3 text-xs">
          ...
          <p className="mt-0.5 text-[11px] text-slate-400">通过等值机组聚类查看逆变器分组与总调节能力，点击机组可展开成员列表。</p>
          <div className="mt-2 space-y-2">
            {groups.map((g) => {
              ...
              return (
                <div key={g.id} className="rounded-lg border border-slate-800 bg-slate-950/80 p-2.5">
                  <button type="button" onClick={() => setExpandedGroupId((prev) => (prev === g.id ? null : g.id))} className="flex w-full items-center justify-between text-left">
                    <div className="space-y-0.5">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="rounded-full bg-slate-800 px-2 py-0.5 text-[10px] text-slate-200">{g.inverterCount} 台逆变器</span>
                      </div>
                      <p className="text-[11px] text-slate-300">ΔP_up={g.totalUp} kW，ΔP_down={g.totalDown} kW，增能力占比 {upPercent.toFixed(0)}%</p>
                    </div>
                    <span className="text-[10px] text-slate-500">{isExpanded ? '收起' : '展开'}</span>
                  </button>
                  {isExpanded && (
                    <div className="mt-2 text-[11px] text-slate-300">
                      <p className="mb-1 text-slate-400">机组内逆变器（示意）：</p>
                      <div className="flex flex-wrap gap-1.5">
                        {Array.from({ length: g.inverterCount }).map((_, idx) => (
                          <span key={`${g.id}-${idx + 1}`} className="rounded-full bg-slate-800 px-2 py-0.5 font-mono text-[10px] text-slate-100">
        ...
        {/* 历史趋势 + 不确定性 + 重新评估 */}
        <section className="space-y-3 rounded-xl border border-slate-800 bg-slate-950/70 p-3 text-xs">
          {/* 历史响应趋势 */}
          <div>
            <h3 className="text-xs font-semibold text-slate-100">响应性能历史趋势（过去 24 小时）</h3>
            <p className="mt-0.5 text-[11px] text-slate-400">折线图展示台区整体响应精度与平均延迟，直方图展示指令误差分布，可用于对比不同设备的响应质量。</p>
            <div className="mt-2 grid gap-3 md:grid-cols-[minmax(0,2fr)_minmax(0,1.4fr)]">
              <div className="h-40 rounded-lg border border-slate-800 bg-slate-950/80 p-2">
                <ResponsiveContainer width="100%" height="100%">
              ...
              <div className="h-40 rounded-lg border border-slate-800 bg-slate-950/80 p-2">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={errorDist} margin={{ top: 10, left: 0, right: 10, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#1f2937" />
                    <XAxis dataKey="bin" stroke="#9ca3af" fontSize={10} angle={-20} textAnchor="end" height={40} />
                    <YAxis stroke="#9ca3af" fontSize={11} label={{ value: '出现次数', angle: -90, position: 'insideLeft', fill: '#9ca3af', fontSize: 10 }} />
                    <Tooltip contentStyle={{ backgroundColor: '#020617', borderColor: '#1f2937', borderRadius: 8, fontSize: 11 }} />
            ...
            <div className="h-40 rounded-lg border border-slate-800 bg-slate-950/80 p-2">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={uncertainty} margin={{ top: 10, left: 0, right: 10, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#1f2937" />
                  <XAxis dataKey="valueKw" stroke="#9ca3af" fontSize={11} label={{ value: 'ΔP_up (kW)', position: 'insideBottom', offset: -5, fill: '#9ca3af', fontSize: 10 }} />
                  <YAxis stroke="#9ca3af" fontSize={11} label={{ value: '相对概率', angle: -90, position: 'insideLeft', fill: '#9ca3af', fontSize: 10 }} />
                  <Tooltip contentStyle={{ backgroundColor: '#020617', borderColor: '#1f2937', borderRadius: 8, fontSize: 11 }} />
                  <Area type="monotone" dataKey="probability" stroke="#a855f7" fill="#a855f7" fillOpacity={0.3} name="概率密度" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
            <div className="flex flex-col justify-between rounded-lg border border-slate-800 bg-slate-950/80 p-3 text-[11px]">
              <div>
              ...
              <div className="mt-3 rounded-md border border-slate-800 bg-slate-950 px-2.5 py-2 text-slate-400">
                <p>
